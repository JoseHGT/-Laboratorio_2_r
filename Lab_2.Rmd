...
join_air_fligths <- flights_selected %>%
  left_join(airlines, by = c("dest" = "carrier"))%>%
  select(-origin, -dest)%>%
...
ggplot(precip_delay, aes(x=precip,y=delay)) +
  geom_point()+
  scale_x_continuous(labels = scales::percent) #Añadiendo porcentaje al eje X
